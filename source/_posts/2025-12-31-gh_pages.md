...
categories: 开发环境配置
toc: true
excerpt: GitHub Pages + Hexo 创建博客流程
---
# 安装hexo
...
## 创建Token
1. 在`GitHub Settings-->Developer settings-->Personal access tokens-->Tokens (classic)-->Generate new tokens (classic)`创建一个token用于Github Action。
2. Note可命名为`hexo_token`，配置中勾选`repo`和`workflow`后生成token。
3. 将token复制到`仓库设置-->Secrets and variables-->Actions-->Repository secrets`。
